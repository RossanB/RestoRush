	...
	if recipe_check["success"]:
		...
		for recipe_result in Recipes.RECIPES.keys():
			...
			for step in recipe["steps"]:
				if step.has("station") and step["station"] == "oven" and step.has("action"):
...
func process_items(player: Node):
	...
	print("Oven stored items: ", stored_items)
	...
	var process_type = "Cooking..."
	for recipe_result in Recipes.RECIPES.keys():
		var recipe = Recipes.RECIPES[recipe_result]
		for step in recipe["steps"]:
			if step.has("station") and step["station"] == "oven" and step.get("result", -1) == result_item:
